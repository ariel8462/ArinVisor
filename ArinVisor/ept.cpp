...
void Ept::setup_ept() noexcept
{
	...
	for (int i = 0; i < kPageEntryCount; i++)
	{
		...
		vcpu_->paging_structs.pdpt[i].bits.pfn = MmGetPhysicalAddress(&vcpu_->paging_structs.pd[i]).QuadPart / PAGE_SIZE;
	}
	...
	for (int pdpt_entry = 0; pdpt_entry < kPageEntryCount; pdpt_entry++)
	{
		...
		{
			vcpu_->paging_structs.pd[pdpt_entry][pd_entry].bits.write_access = true;
			vcpu_->paging_structs.pd[pdpt_entry][pd_entry].bits.read_access = true;
			vcpu_->paging_structs.pd[pdpt_entry][pd_entry].bits.execute_access = true;
			vcpu_->paging_structs.pd[pdpt_entry][pd_entry].bits.memory_type = 6;  // write-back
			vcpu_->paging_structs.pd[pdpt_entry][pd_entry].bits.large_page = true;
			vcpu_->paging_structs.pd[pdpt_entry][pd_entry].bits.pfn = static_cast<unsigned long long>(
